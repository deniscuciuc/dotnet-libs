...
{
    public Task EnqueueAsync<TJob>(CancellationToken ct) where TJob : IPlatformJob
    {
    ...
    public Task EnqueueAsync<TJob>(object args, CancellationToken ct) where TJob : IPlatformJob
    {
    ...
    public Task ScheduleAsync<TJob>(TimeSpan delay, CancellationToken ct) where TJob : IPlatformJob
    {
    ...
    public Task ScheduleAsync<TJob>(object args, TimeSpan delay, CancellationToken ct) where TJob : IPlatformJob
    {
        ...
        where TJob : IPlatformJob
    {
        ...
        var queue = typeof(TJob).GetCustomAttribute<PlatformJobAttribute>()?.Queue ?? "default";
        var typeName = typeof(TJob).AssemblyQualifiedName!;
    ...
    public async Task ExecuteJob(string jobTypeAssemblyQualifiedName, CancellationToken ct)
    {
        ...
        var job = (IPlatformJob)scope.ServiceProvider.GetRequiredService(jobType);
    ...
    public async Task ExecuteJobWithArgs(string jobTypeAssemblyQualifiedName, object? args, CancellationToken ct)
    {
        ...
        if (args is not null && job is IPlatformJob<object> typedJob)
            typedJob.Args = args;
