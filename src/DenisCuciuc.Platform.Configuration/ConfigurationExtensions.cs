...
{
    /// <summary>
    /// Registers the DenisCuciuc.Platform opinionated YAML configuration pipeline.
    /// Layers are added in order: base → environment-specific → local override → environment variables.
    /// </summary>
    public static IHostApplicationBuilder AddPlatformConfiguration(
        this IHostApplicationBuilder builder,
        Action<PlatformConfigurationOptions>? configure = null)
    {
        var options = new PlatformConfigurationOptions();
    ...
    /// <para>
    /// Convention: strip the <c>Options</c> suffix and any leading <c>QG</c> or <c>Platform</c> prefix from the type name,
    /// then use the remaining type name as the section path.
    /// Examples: <c>MongoOptions</c> → <c>Mongo</c>, <c>PlatformSentryOptions</c> → <c>Sentry</c>.
    /// </para>
    ...
    /// </param>
    public static OptionsBuilder<T> AddPlatformOptions<T>(
    ...
    /// should leave <see cref="PlatformConfigurationOptions.Profile"/> at Default and set
    /// the individual Include* flags directly.
    /// </summary>
    private static void ApplyProfile(PlatformConfigurationOptions options)
    {
    ...
    /// <remarks>
    /// Convention: strip trailing <c>Options</c> suffix, strip leading <c>QG</c> or <c>Platform</c> prefix, then use the remaining type name as-is.
    /// <c>MongoOptions</c> → <c>Mongo</c> | <c>PlatformSentryOptions</c> → <c>Sentry</c>
    /// </remarks>
    private static string ResolveSection<T>()
    {
        ...
        if (name.StartsWith("QG", StringComparison.Ordinal) && name.Length > 2)
            name = name[2..];

        if (name.StartsWith("Platform", StringComparison.Ordinal) && name.Length > "Platform".Length)
