...
public sealed class InMemoryJobScheduler : IJobScheduler
{
    ...
    public Task EnqueueAsync<TJob>(CancellationToken ct = default) where TJob : IPlatformJob
    {
        EnqueuedJobs.Enqueue(new EnqueuedJobRecord(typeof(TJob), null));
        return Task.CompletedTask;
    }

    public Task EnqueueAsync<TJob>(object args, CancellationToken ct = default) where TJob : IPlatformJob
    {
        EnqueuedJobs.Enqueue(new EnqueuedJobRecord(typeof(TJob), args));
        return Task.CompletedTask;
    }

    public Task ScheduleAsync<TJob>(TimeSpan delay, CancellationToken ct = default) where TJob : IPlatformJob
    {
