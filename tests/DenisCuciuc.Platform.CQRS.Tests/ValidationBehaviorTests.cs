...
{
    private static IOptionsMonitor<PlatformCqrsOptions> BuildOptions(bool enabled = true)
    {
        var monitor = Substitute.For<IOptionsMonitor<PlatformCqrsOptions>>();
        monitor.CurrentValue.Returns(new PlatformCqrsOptions { EnableValidationBehavior = enabled });
        return monitor;
    }
