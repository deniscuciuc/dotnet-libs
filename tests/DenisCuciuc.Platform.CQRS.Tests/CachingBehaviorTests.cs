...
{
    private static IOptionsMonitor<PlatformCqrsOptions> BuildOptions(
        bool cachingEnabled = true, bool behaviorEnabled = true)
    {
        var monitor = Substitute.For<IOptionsMonitor<PlatformCqrsOptions>>();
        monitor.CurrentValue.Returns(new PlatformCqrsOptions
        {
            EnableCachingBehavior = behaviorEnabled,
            Caching = new PlatformCqrsCachingOptions
            {
