...
public interface IJobScheduler
{
    ...
    Task EnqueueAsync<TJob>(CancellationToken ct = default) where TJob : ICoreJob;

    /// <summary>Enqueues a job for immediate execution with arguments.</summary>
    Task EnqueueAsync<TJob>(object args, CancellationToken ct = default) where TJob : ICoreJob;

    /// <summary>Schedules a job to execute after the specified delay.</summary>
    Task ScheduleAsync<TJob>(TimeSpan delay, CancellationToken ct = default) where TJob : ICoreJob;

    /// <summary>Schedules a job to execute after the specified delay with arguments.</summary>
    Task ScheduleAsync<TJob>(object args, TimeSpan delay, CancellationToken ct = default) where TJob : ICoreJob;

    /// <summary>Adds or updates a recurring job with the given cron expression.</summary>
    Task AddOrUpdateRecurringAsync<TJob>(string jobId, string cron, CancellationToken ct = default) where TJob : ICoreJob;
