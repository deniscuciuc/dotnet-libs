...
// Register startup pipeline (from CoreLibs.Redis.Startup)
services.AddCoreRedisStartup();
```

### Startup (via `CoreLibs.Redis.Startup`)

`AddCoreRedisStartup()` registers `RedisConnectStartup` (order 0) which connects all `IRedisConnection` instances in parallel and marks the `StartupGuard<IRedisClient>` as configured.
...
## Cache abstractions (`CoreLibs.Cache`)

`CoreLibs.Cache` owns all abstractions; `CoreLibs.Redis` provides the implementation.
