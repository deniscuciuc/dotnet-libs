...
// Register startup pipeline (from CoreLibs.MongoDB.Startup)
services.AddCoreMongoDBStartup();
```

### Startup (via `CoreLibs.MongoDB.Startup`)

`AddCoreMongoDBStartup()` registers four `IStartup` tasks that run automatically via `RunWithStartupAsync()`:
