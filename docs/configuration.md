# DenisCuciuc.Platform.Configuration
...
| Layer | File | Optional |
| --- | --- | --- |
| Base | `appsettings.yml` | Yes |
| Environment-specific | `appsettings.{ASPNETCORE_ENVIRONMENT}.yml` | Yes |
| Environment variables | no prefix | n/a |
...
```csharp
builder.AddPlatformConfiguration(options =>
{
    ...
    options.EnvironmentVariablesPrefix = null; // default; set a prefix to scope env vars
});
```
...
| `EnvironmentVariablesPrefix` | `string?` | `null` | Prefix filter for env vars; `null` loads all |
...
| --- | --- | --- | --- | --- |
| `Default` | Yes | Yes | per flag | Yes |
| `Minimal` | Yes | No | No | Yes |
| `Cloud` | Yes | Yes | No | Yes |
| `LocalDev` | Yes | Yes | Yes | Yes |
...
`AddPlatformOptions<T>` is the primary way to bind configuration sections to strongly-typed classes.
...
services.AddPlatformOptions<MongoOptions>(builder.Configuration);
services.AddPlatformOptions<RedisOptions>(builder.Configuration);

// Explicit section override
services.AddPlatformOptions<MongoOptions>(builder.Configuration, sectionName: "infrastructure:mongo");
```
...
- Set `EnvironmentVariablesPrefix` when you want to scope environment-variable loading to a dedicated prefix
